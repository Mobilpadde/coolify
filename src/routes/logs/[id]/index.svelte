<script context="module" lang="ts">
	...
	export const load: Load = async ({ fetch, params, url, stuff }) => {
		let logsEndPoint = `/applications/${params.id}/logs.json`;
		...
		let buildsEndpoint = `/applications/${params.id}/logs/build.json?skip=0`;
		const buildsRes = await fetch(buildsEndpoint);

		let builds = {};
		if (buildsRes.ok) {
			builds = await buildsRes.json();
		}

		if (logsRes.ok || buildsRes.ok) {
			return {
				props: {
		...
		return {
			status: !logsRes.ok && !buildsRes.ok ? 404 : 500,
...
<script lang="ts">
	...
	let preselectedLogId = $page.url.searchParams.get('logId');
	if (preselectedLogId) logId = preselectedLogId;

	async function updateBuildStatus({ detail }) {
		const { status } = detail;
		if (status !== 'running') {
			try {
				const data = await get(`/applications/${id}/logs/build.json?buildId=${logId}`);
			...
			builds = builds.filter((build) => {
				if (build.id === logId) build.status = status;
				return build;
			});
		}
	}
	...
	function loadBuild(build) {
		logId = build;
		return changeQueryParams(logId);
	}
...
<div class="flex items-center space-x-2 p-5 px-6 font-bold">
	<div class="-mb-5 flex-col">
		<div class="md:max-w-64 truncate text-base tracking-tight md:text-2xl lg:block">Logs</div>
...
<div class="block flex-row justify-start space-x-2 px-5 pt-6 sm:px-10 md:flex">
		...
		<div class="top-4 md:sticky">
			{#each builds as build, index (build.id)}
				<div
					...
					class:bg-coolgray-400={logId === build.id}
	...
	<div class="flex-1 md:w-96">
		{#if logId}
			{#key logId}
				<svelte:component this={Log} {logId} {logsEl} on:updateBuildStatus={updateBuildStatus} />
			{/key}
		{/if}
	</div>
</div>
